...
Now let's create a file named `[app.py](http://app.py)`, this is where our backend server is served.
...
In the `Api.js` you'll see that the call is made through a variable named `HOST` , this link is specifically separated out because we will be changing its value when the project is deployed to the Heroku server. Now the frontend is fetching from `[localhost:5000](http://localhost:5000)` which is the port flask runs on, in the later chapters we will change it to our live website domain.
